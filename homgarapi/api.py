...
from .devices import MODEL_CODE_MAPPING, HomgarHome, HomgarHubDevice
...
class HomgarApi:
    ...
    def login(self, email: str, password: str, area_code="31") -> None:
        ...
        self.cache["token_expires"] = datetime.utcnow().timestamp() + data.get(
            "tokenExpired"
        )
    ...
    def get_devices_for_hid(self, hid: str) -> list[HomgarHubDevice]:
        ...
        def get_device_class(dev_data):
            ...
            if model_code not in MODEL_CODE_MAPPING:
                logger.warning(
                    "Unknown device '%s' with modelCode %d",
                    dev_data.get("model"),
                    model_code,
                )
                return None
    ...
    def ensure_logged_in(
        ...
        """
        if self.cache.get("email") != email or datetime.fromtimestamp(
            self.cache.get("token_expires", 0)
